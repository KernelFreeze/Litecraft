...
void imguiInit(GLFWwindow* window, bool install_callbacks = true) {
    ...
    ImFontConfig config;
    auto font = resourcePath("font.ttf", "fonts", "litecraft").toStringz;

    // Default range
    ImFontAtlas_AddFontFromFileTTF(io.Fonts, font, 18, &config,
    ...
    // Cyrillic
    ImFontAtlas_AddFontFromFileTTF(io.Fonts, font, 18, &config,
            ImFontAtlas_GetGlyphRangesCyrillic(io.Fonts));

    // Include full set of about 21000 CJK Unified Ideographs
    ImFontAtlas_AddFontFromFileTTF(io.Fonts, font, 18, &config,
            ImFontAtlas_GetGlyphRangesJapanese(io.Fonts));

    // Default + Hiragana, Katakana, Half-Width, Selection of 1946 Ideographs
    ImFontAtlas_AddFontFromFileTTF(io.Fonts, font, 18, &config,
            ImFontAtlas_GetGlyphRangesChinese(io.Fonts));
